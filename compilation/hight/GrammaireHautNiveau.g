...
nouveauType @init{boolean valide = true; GestionTypes tmp = new GestionTypes();} :
  'type' t=ident {
			if(gt.isTypeDeBase($t.text) || gt.isTypeDeclare($t.text)) {
				System.err.println("Erreur : récursion pour le type "+$t.text+".");
				valide = false;
			} 
	} 'is' (s1=ident {
			if(!gt.isTypeDeclare($s1.text)){
				System.err.println("Erreur : le type "+$s1.text+" n'existe pas.");
				valide = false;
				}
			} 
	| ti=typeObjet {
		if(!tmp.isTypeDeclare($ti.text)){
				tmp.ajouterType($ti.text);
			}
		else System.err.println("Erreur : le nouveau type "+$t.text+ " hérite deux fois de la classe "+$ti.text+".");}
	
	)  ('and' (si=ident {
			if(!gt.isTypeDeclare($si.text)) {
				System.err.println("Erreur : le type "+$si.text+" n'existe pas.");
				valide = false;
				if(!tmp.isTypeDeclare($si.text)){
					tmp.ajouterType($si.text);
					}
				else System.err.println("Erreur : le nouveau type "+$t.text+ " hérite deux fois de la classe "+$si.text+".");
				}
			}
	| ti=typeObjet {
		if(!tmp.isTypeDeclare($ti.text)){
	...
	}))*  { if(valide) gt.ajouterType($t.text);  } // to declare a new type
  ;            
