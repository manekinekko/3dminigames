...
La syntaxe est la même. Ainsi, \code{Mario has energie at 8} ajoute un attribut energie (qui n'existe pas déjà dans Character ou Object) à Mario,
 initialisé à 8.
...
Par exemple \code{Koopa is} \code{Character enemy duplicable. generate 5 Koopa in zone}. 
...
Par exemple, en reprenant l'exemple de Mario : \code{Mario is Character player. definition sursautMario means mario jump 15.}
...
\code{command mario is key space for jump 15,} \code{ key Z for move forward, key Y for move backward, key S for move left, key D for move right.}
...
\code{rule mario touches bowser then sub 1 for life of} \code{ mario, sub 100 for score of game, mario jump 15}.
...
\code{foret\{chene,olivier,peuplier\}}
...
Voici l'exemple  d'un jeu où un personnage \code{perso} et un ennemi \code{boss} ont été définis, ainsi que des ressources \code{persoLife} et \code{bossLife}.
Dans le cas où le signal \code{bossAttack} est lancé, il est possible d'avoir les règles :
\code{bossAttack persoLife -1} qui retranche 1 à la ressource \code{marioLife}.
Puisque \code{persoLife} est modifié, le signal \code{updatePersoLife} est émis et on peut avoir une règle
\code{updatePersoLife if persoLife==0 then gameOver}.
